...
class SonameCache:
    ...
    def __getitem__(self, key: tuple[_ElfArchitectureTuple, str]) -> Path:
        """Obtain cached item."""
        return self._soname_paths[key]

    def __setitem__(self, key: tuple[_ElfArchitectureTuple, str], item: Path) -> None:
        """Add item to cache."""
        # Initial API error checks
        if not isinstance(key, tuple):
            raise OSError("The key for SonameCache has to be a (arch, soname) tuple.")
        if not isinstance(key[0], tuple) or len(key[0]) != 3:  # noqa: PLR2004
            raise OSError(
...
class ElfFile:
    ...
    def _extract_attributes(  # noqa: PLR0912 (too-many-branches)
        ...
        with self.path.open("rb") as file:
            ...
            #
            # For amd64 binaries, this will evaluate to: ('ELFCLASS64',
            # 'ELFDATA2LSB', 'EM_X86_64')
            self.arch_tuple = (
...
def _ldd(
    ...
    ldd = utils.get_host_tool("ldd")  # Should use `ld` from the base snap (#4751)
    env = {
...
def _ldd_resolve(soname: str, soname_path: str) -> tuple[str, str]:
    ...
    if soname_path.startswith("/") and Path(soname_path).exists():
        abs_path = os.path.abspath(soname_path)  # noqa: PTH100
