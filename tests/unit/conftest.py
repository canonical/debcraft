...
import debcraft
import debcraft.services.helper
...
def fake_project_service_class(
    ...
    class FakeProjectService(debcraft.services.project.Project):
        @override
        def _load_raw_project(self) -> dict[str, Any]:  # pyright: ignore[reportIncompatibleMethodOverride] # type: ignore[reportIncompatibleMethodOverride,ty:override-of-final-method]
        ...
        def set(self, value: models.Project) -> None:
            ...
            self._platform = next(
                iter(value.platforms)  # pyright: ignore[reportCallIssue,reportArgumentType] # ty: ignore[no-matching-overload]
            )
            self._build_for = value.platforms[self._platform].build_for[0]  # pyright: ignore[reportOptionalSubscript] # type: ignore[reportOptionalSubscript,ty:not-subscriptable]
...
@pytest.fixture
def fake_helper_service_class(tmp_path, host_architecture):
    class FakeHelperService(debcraft.services.helper.HelperService):
        def __init__(
            self,
            app: craft_application.AppMetadata,
            services: services.ServiceFactory,
...
def default_factory(
    ...
    services.ServiceFactory.register("helper", fake_helper_service_class)
...
@pytest.fixture
def helper_service(default_factory) -> debcraft.services.helper.HelperService:
    return cast(debcraft.services.helper.HelperService, default_factory.helper)
